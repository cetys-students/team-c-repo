...
##        self.LPf = LP(self.get_accelerometer_cal(),0.8)
    ...
    def enable(self, accelerometer=True, gyroscope=True):
        ...
        if gyroscope:
            ...
            self.write_register(L3GD20H_Address, L3GD20H.CTRL4, 0x20)
...
##        LPf = self.LPf
    ...
    def get_gyroscope_average(self, average=3):
        x = []
        y = []
        z = []
        for i in range(average):
            gyro = self.get_gyroscope_cal()
            x.append(gyro[0])
            y.append(gyro[1])
            z.append(gyro[2])
        aveX = sum(x)/average
    ...
    def get_bias_gyro(self):
        self.HPf = HP(self.get_gyroscope_average(average=5),0.20)
        CompaX = []
        CompaY = []
        CompaZ = []
        for i in range(100):
            oieHP = self.HPf.update_measurement(self.get_gyroscope_average(average=5))
            #print(oieHP)
        ...
        for i in range(100):
            oieHP = self.HPf.update_measurement(self.get_gyroscope_average(average=5))
            #print(oieHP)
            CompaX.append(oieHP[0])
            CompaY.append(oieHP[1])
            CompaZ.append(oieHP[2])
        x = sum(CompaX)/100
        y = sum(CompaY)/100
        z = sum(CompaZ)/100
        #print([x,y,z])
    ...
    def get_regulated_value(self,Bias):
        EseWey = self.HPf.update_measurement(self.get_gyroscope_average(average=5))
        #print(EseWey)
        X = EseWey[0]-Bias[0]
        Y = EseWey[1]-Bias[1]
        Z = EseWey[2]-Bias[2]
